...
class LoginForm extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Has superado el número máximo permitido de intentos... favor de esperar ".$timeout_minutes." minutos antes de intentarlo otra vez</div>";
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
                ...
                $success = 'true';
                    ...
                    $_SESSION['foto'] = $result['foto_usuaro'];
                ...
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>ERROR, usuario o contraseña no correctos</div>";
